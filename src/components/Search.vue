<template>
  <el-row draggable="false">
    <search-start v-on:StartSearch="StartSearch($event)" />
...
acgdata = item;
          ...
          <a target="_blank">
            <el-card style="border: none" :body-style="{ padding: '0px', background: type_color(item.AID) }">
              <div style="position: relative">
                <img draggable="false" v-bind:src="item.R封面图小" class="image" width="100%" :onerror="defalutLogoUrl"
    ...
    <el-col>
      <transition name="el-zoom-in-center">
        <el-empty v-if="nohaveData" :image="defalutNodataIMG" description="没有找到哦"></el-empty>
      </transition>
    </el-col>
...
export default {
  components: { SearchStart, AltConnectVue },
  name: "Search_",
  Info_box,
  props: {
    Data_type: String,
  ...
  data() {
    return {
      ...
      defalutNodataIMG: DefaultData.nodataimg,
      josn_data: JSON,
    ...
    StartSearch(keyword) {
      ...
      axios
        ...
        .then((response) => {
          ...
          _this.MAXnum = response.data.SeaCnt;
          _this.runstart(response.data.AniPreL, 1);
          setTimeout(function () {
            _this.autoadd();
            _this.setatag();
          }, 500);
  ...
  mounted() {
    ...
    window.onresize = () => {
      if (this.Search_word != "") this.autoadd();
    };
